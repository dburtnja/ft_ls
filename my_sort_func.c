...
/*   Updated: 2018/01/13 13:26:14 by dburtnja         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			by_name(t_node *first, t_node *second, int type)
{
	...
	if (type == TYPE_DIR)
	{
		first_name = ft_strmap(((t_dir*)first->data)->dir_with_path,
				&my_to_lower);
		second_name = ft_strmap(((t_dir*)second->data)->dir_with_path,
				&my_to_lower);
	}
	else if (type == TYPE_FILE)
	{
		first_name = ft_strmap(((t_file*)first->data)->file_name, &my_to_lower);
		second_name = ft_strmap(((t_file*)second->data)->file_name,
				&my_to_lower);
	}
	if (type == TYPE_FILE || type == TYPE_DIR)
		return (my_strcmp((unsigned char *)first_name,
